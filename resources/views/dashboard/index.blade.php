...
<html lang="en">
  <head>
    ...
    <title>Dashboard | Hotel Reviews</title>
      ...
      <div class="content-container-1">
        <div class="content-container-2">
          <div class="box-1">
            ...
            <h3>{{ $user->name }}</h3>
            <h3>{{ $user->review->count() }} Review(s)</h3>
            ...
            <label for="overlay-checkbox" id="edit-button">Edit</label>

            <!-- Invisible checkbox that controls the overlay visibility -->
            <input type="checkbox" id="overlay-checkbox" />

            <!-- Overlay -->
            <div class="overlay">
              <!-- Box within the overlay containing the form -->
              <div class="overlay-box">
                <!-- Add your form content here -->
                <label for="overlay-checkbox" id="exit-button">X</label>
                <h1>Edit Account</h1>
                <div class="edit-detail-container">
                  <form method="post" action="/dashboard">
                    @csrf
                    <div class="edit-detail">
                      <p style="color: #80b3ff">Username</p>
                      <input type="text" class="edit-input-box" required name="name" value="{{ $user->name }}"/>
                    </div>
                    <div class="edit-detail">
                      <p style="color: #80b3ff">Email</p>
                      <input type="text" class="edit-input-box" required name="email" value="{{ $user->email }}"/>
                    </div>
                    <div class="edit-detail">
                      <p style="color: #80b3ff">Phone Number</p>
                      <input type="text" class="edit-input-box" required name="number" value="{{ $user->number }}"/>
                    </div>
                    <button class="detail-save-button">Save</button>
                  </form>
                </div>
              </div>
            </div>
